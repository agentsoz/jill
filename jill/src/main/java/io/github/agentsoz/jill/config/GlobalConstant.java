...
public class GlobalConstant {
	...
	/**
	 * Available policies for selecting between plan instances.
	 * FIRST: selects the first plan instance from the list of applicable instances
	 * RANDOM: selects randomly from the list of applicable instances
	 * LAST: selects the last plan instance from the list of applicable instances (for test purposes only)
	 * @author dsingh
	 *
	 */
	 ...
	 * policy is RANDOM, then this number controls the maximum number
	 * of instances to draw from. For instance, say 
	 ...
	 * 90 instances.
	 * 
	 * This limit can be changed at runtime using the --plan-instances-limit
	 * option. Increasing this limit, however, may have a significant 
	 * performance hit, so should be done only if necessary. 
	 */
	public static int PLAN_INSTANCES_LIMIT = 10;

	/**
	 * If set to true (default), causes the system to exit when it goes into the 
	 * idle state, i.e., when the intention stack becomes empty. Setting this
