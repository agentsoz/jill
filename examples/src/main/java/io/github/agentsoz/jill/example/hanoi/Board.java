...
public class Board {
	...
	public boolean move (int a, int b) {
		...
		if (a < 0 || a >= pins.length || b < 0 || b >= pins.length) {
			Log.warn("Invalid board pin specified " + a + ". Should be between 0.."+(pins.length-1)+" (inclusive).");
			return false;
		...
		if (discOnB < discOnA) {
			Log.warn("Cannot move disc" + discOnA + " (pin"+a+") on to smaller disc" + discOnB + " (pin"+b+")");
			return false;
		}
